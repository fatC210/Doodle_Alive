...
export function buildAgentPayload(payload: DidRequestBody, sourceUrl: string) {
  ...
  return {
    ...
    presenter: buildPresenterConfig(llm.provider, sourceUrl, normalizeAgentLanguage(payload.language)),
    llm,
...
function buildPresenterConfig(llmProvider: string, sourceUrl: string, language: DidAgentLanguage) {
  if (llmProvider === 'd-id') {
    return {
      type: 'expressive',
